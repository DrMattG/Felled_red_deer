...
# url for POST
url <- "https://data.ssb.no/api/v0/en/table/03434/"
...
data<-'{
  "query": [
    {
      "code": "Region",
      "selection": {"filter": "agg:KommSummer",
"values": [
  "K-3001",
  "K-3002",
  "K-3003",
...
title <- tags$a(href='https://www.nina.no',
                ...
                'Felled red deer, Norway', target="_blank")
#UI
ui <- dashboardPage(
  dashboardHeader(
  ...
  dashboardSidebar(
    sidebarMenu(
      menuItem("Dashboard", tabName = "Felled red deer, Norway", icon = icon("dashboard")),
    ...
    tabsetPanel(
      ...
      tabPanel(
        ...
        fluidRow(box(title= "Select inputs"
                     ...
                     ,collapsible = TRUE
                     ,width = 6
                     , selectInput("age", "Choose the reported age category:",
                                   c("Total"= "Total",
                                     "Calf" = "Calf",
                                     "Yearlings" = "Yearlings",
                                     "Older" = "Older")),
                     selectInput("sex", "Choose the focal sex",
                                 c("Both"="Both sexes", "Males"="Males", "Females"="Females")),
                     selectInput("region", "Choose the region"
                                 ,choices=unique(dattable$region)
                                 ,multiple = TRUE
                                 ,selected = "Kongsberg") 
        ),
      ...
      fluidRow(
        ...
        box(title = "Region"
            ...
            ,solidHeader = TRUE
            ,collapsible = TRUE
            ,plotOutput("region", height = "500px"))
      )
    )
  )
)))
...
server <- function(input, output) {
  ...
  output$mortality <- renderPlot({
    ggplot(data = data(),
      ...
      ggtitle(as.character(input$sex))+
  ...
  output$region <- renderPlot({
    ggplot(data = Rdata(),
      ...
      theme_classic()+
      theme(axis.text.x = element_text(angle = 90, hjust = 1))+
      ggtitle(paste0(as.character(input$region), ": ", as.character(input$sex), ", ",as.character(input$age) ))+
      theme(plot.title = element_text(hjust = 0.5))
  })
}
#Launch the App
